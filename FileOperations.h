...
void outputGenerator(string filename, stack<Location*> cities, stack<Route*> routes, bool costOrTime){
    ...
    output<<"<html><head><title>Route Map</title></head>"
          <<"<script src='https://maps.googleapis.com/maps/api/js'></script>"
          <<"<script>function init(){"
          <<"var map=new google.maps.Map(document.getElementById('map'),{zoom:5,center:{lat:20.5937,lng:78.9629}});";

    int i=0;

    while(!cities.empty() && !routes.empty()){
        Location* origin=cities.top();cities.pop();
        Location* destination=cities.top();

        Route* route=routes.top();routes.pop();

        // markers
        output<<"var m"<<i<<"=new google.maps.Marker({position:{lat:"<<origin->lat<<",lng:"<<origin->lon<<"},map:map,title:'"<<origin->capital<<"'});";
        output<<"var m"<<i+1<<"=new google.maps.Marker({position:{lat:"<<destination->lat<<",lng:"<<destination->lon<<"},map:map,title:'"<<destination->capital<<"'});";

        // path
        output<<"var path"<<i<<"=new google.maps.Polyline({path:[{lat:"<<origin->lat<<",lng:"<<origin->lon<<"},{lat:"<<destination->lat<<",lng:"<<destination->lon<<"}],strokeColor:'#FF0000'});";
        output<<"path"<<i<<".setMap(map);";
